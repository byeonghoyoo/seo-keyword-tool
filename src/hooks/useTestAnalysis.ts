...
export function useTestAnalysis() {
  ...
  const startProgressTracking = useCallback((jobId: string) => {
    ...
    eventSource.onopen = () => {
      console.log('Test progress tracking connected');
    };
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        switch (data.type) {
          case 'connected':
            console.log('Connected to test progress stream');
            break;
        ...
        console.error('Failed to parse test progress data:', error);
      }
    };
    ...
    eventSource.onerror = (error) => {
      ...
      setState(prev => ({
        ...prev,
        isLoading: false,
        error: 'Connection to test progress tracking failed',
      }));
      eventSource.close();
    };
  }, []);
